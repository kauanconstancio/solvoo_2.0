...
import { Copy, Check, Clock, QrCode, Loader2, X, CheckCircle2, RefreshCw, Sparkles, Crown } from "lucide-react";
...
interface SubscriptionCheckoutDialogProps {
  ...
  pixData: SubscriptionPixData | null;
  isLoading?: boolean;
  onPaymentConfirmed?: () => void;
}

// Success Animation Component
const SubscriptionSuccessAnimation = ({
  planName,
  price,
  onClose,
}: {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-foreground/60 backdrop-blur-sm animate-fade-in">
      <div className="relative bg-card rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl animate-scale-in border border-border/50">
        ...
        <div className="flex flex-col items-center text-center relative z-10">
          ...
          <h2 className="mt-6 text-2xl font-bold text-foreground">Assinatura Ativada!</h2>

          <p className="mt-2 text-muted-foreground">Bem-vindo ao plano {planName}</p>
...
export const SubscriptionCheckoutDialog = ({
  open,
  onOpenChange,
  pixData,
  isLoading = false,
  ...
  const checkStatus = useCallback(async () => {
    if (!pixData?.subscriptionId || !pixData?.pixId || paymentStatus === "paid") return;

    try {
      setIsCheckingStatus(true);
      const data = await checkPaymentStatus(pixData.subscriptionId, pixData.pixId);
  ...
  useEffect(() => {
    if (!pixData?.subscriptionId || !pixData?.pixId || paymentStatus === "paid" || paymentStatus === "expired") {
      return;
    }
  ...
  }, [pixData?.subscriptionId, pixData?.pixId, paymentStatus, checkStatus]);
          ...
          {isLoading ? (
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="bg-muted/50 rounded-lg p-4">
                ...
                <p className="font-medium">{pixData.planName}</p>
                <p className="text-2xl font-bold text-primary mt-1">
                  {formatPrice(pixData.planPrice)}
