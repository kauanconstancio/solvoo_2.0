...
export const PixBookingCheckoutDialog = ({
  open,
  onOpenChange,
  appointmentId,
  pixData,
  ...
  const checkPaymentStatus = useCallback(async () => {
    if (!appointmentId || !pixData?.pixId || paymentStatus === "paid") return;

    try {
      setIsCheckingStatus(true);
      const { data, error } = await supabase.functions.invoke("check-booking-payment", {
        body: { appointmentId },
      });
  ...
  useEffect(() => {
    if (!appointmentId || !pixData?.pixId || paymentStatus === "paid" || paymentStatus === "expired") {
      return;
    }
  ...
  }, [appointmentId, pixData?.pixId, paymentStatus, checkPaymentStatus]);
